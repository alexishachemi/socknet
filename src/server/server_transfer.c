...
static bool can_send(net_t *net, int sock, const void *data, unsigned int n)
{
    if (!net || sock < 0 || !data || n == 0)
        return FALSE_NLOG(net, ERROR, "server failed to send data");
    if (n > MAX_PACKET_SIZE)
        return FALSE_NLOG(
            net, ERROR,
            "server failed to send data, packet too big (n: %d, max: %d)",
            n, MAX_PACKET_SIZE
        );
    return true;
}
...
bool nets_send(net_t *net, int sock, const void *data, unsigned int n)
{
    ...
    return TRUE_NLOG(net, INFO, "server sent data to client %d", sock);
}
...
bool nets_recv(net_t *net, net_transfer_t *t_buf)
{
    ...
    if (!net || !t_buf || !net_has_to_recv(net))
        return FALSE_NLOG(net, ERROR, "server failed to receive data");
    transfer = list_pop(&net->to_recv, 0);
    memcpy(t_buf, transfer, sizeof(net_transfer_t));
    free(transfer);
    return TRUE_NLOG(net, INFO, "server received data from client %d",
